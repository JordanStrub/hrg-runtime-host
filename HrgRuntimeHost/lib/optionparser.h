		...
		public:
			...
			char* error_msg()
			{
				if (_last_error_arg != nullptr)
					sprintf_s(_message, MessageBufferLength, "%s: %s", _last_error_arg, _last_error.c_str());
				else if (_last_error_desc >0)
					sprintf_s(_message, MessageBufferLength, "--%s or %c%s: %s", _pDescriptor[_last_error_desc].longopt, _short_delimiter, _pDescriptor[_last_error_desc].shortopt, _last_error.c_str());
				return _message;
			}
