...
namespace aristocrat
{
	...
	private:
		...
		{
			if (strcmp(pSymInfo->Name, "this") != 0) return TRUE;
			if (pSymInfo != 0)
			{
				WCHAR* pType_name = 0;
				DWORD dwType = 0;
		...
		bool Initialize()
		{
			...
			if (::EnumProcessModules(_hProcess, hMods, sizeof(hMods), &cbNeeded))
			{
				...
				for (i = 0; i < (cbNeeded / sizeof(HMODULE)); i++)
				{
					...
					{
						char szFullPathModName[MAX_PATH];
						::GetFullPathNameA(szModName, MAX_PATH, szFullPathModName, nullptr);
						if (SymLoadModule64(_hProcess, 0, szFullPathModName, szModBaseName, (DWORD64)mi.lpBaseOfDll, mi.SizeOfImage))
						{
	...
	public:
		std::vector<DWORD64>	Modules() { return _module_bases; };
		void Release()
		{
			delete this;
		}
		...
		static SymbolLookup* Create(HANDLE hProcess = 0)
		{
			...
			static char szExeFolder[MAX_PATH];
			
			GetModuleFileNameA(NULL, szExeFolder, MAX_PATH);
			PathRemoveFileSpecA(szExeFolder);

			if (hProcess == 0)	hProcess = GetCurrentProcess();


			// Add Symbol Search paths (separated by semicolon)
			std::string searchPaths;
			char directory[MAX_PATH];
			::GetCurrentDirectoryA(MAX_PATH, directory); // add current working directory
			searchPaths.append(directory);
			searchPaths.append(";");
			GetModuleFileNameA(NULL, directory, MAX_PATH);
			PathRemoveFileSpecA(directory);
			searchPaths.append(directory);   // add executable directory


			if (symInitialize(hProcess, (PSTR)searchPaths.c_str(), false) == FALSE)
			{
				::FreeLibrary(hDbgHelp);
				return nullptr;
			}
